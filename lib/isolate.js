...
const { addSyncGlobal, addAsyncGlobal } = require('./vm-util')
const { createApplyTransactor } = require('./guest-apis/apply')
const { addContractIndexGlobal } = require('./guest-apis/contract-index')
const { addContractOplogGlobal } = require('./guest-apis/contract-oplog')

const DISALLOWED_RESTRICTED_METHODS = [
  '__internal__.contractOplog.append',
]
...
exports.ItoEsmIsolate = class ItoEsmIsolate extends EventEmitter {
  ...
  async _configureEnvironment () {
    ...
    if (this._globals.__internal__?.contractIndex) {
      await addContractIndexGlobal(context, this._globals.__internal__.contractIndex)
      await context.eval(`global.index = new ContractIndex("${this._indexPubkey}")`)
    }
    if (this._globals.__internal__?.contractOplog && typeof this._oplogPubkey === 'string') {
      await addContractOplogGlobal(context, this._globals.__internal__.contractOplog)
      await context.eval(`global.oplog = new ContractOplog("${this._oplogPubkey}")`)
    }
