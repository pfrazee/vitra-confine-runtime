...
const { APIDescription, MethodNotFound } = require('abstract-confine-runtime')
const ivm = require('@andrewosh/isolated-vm')
const ImportController = require('./imports')
const uuid = require('uuid')
const assert = require('commonjs-assert').strict
...
exports.VitraEsmIsolate = class VitraEsmIsolate extends EventEmitter {
  ...
  async handleAPICall (methodName, params) {
    return await this._restrictedGuard('handleAPICall', async () => {
      if (methodName === 'apply') {
        ...
        const op = params[0]
        const ack = params[1]
        let methodRef = this.module.namespace.getSync('apply', {reference: true})
        if (methodRef?.typeof === 'object' && typeof op?.op === 'string') {
          methodRef = methodRef.getSync(op.op, {reference: true})
        }
        if (methodRef?.typeof === 'function') {
          const {actions, tx} = createApplyTransactor()
          await methodRef.apply(null, [tx, op, ack], {
            arguments: {copy: true},
